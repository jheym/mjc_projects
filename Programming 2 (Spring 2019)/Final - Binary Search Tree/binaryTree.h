...
private:
    ...
    int singleP(nodeType<elemType> *p) const;
      // Function that determines the number of single parents of a binary tree
      // A single parent node is one that only has one child

    void swapSubtreesOfNode(nodeType<elemType> *p);
      // Swap all left and right subtrees of a binary tree
      // This is done at every node.
...
int binaryTreeType<elemType>::singleP(nodeType<elemType> *p) const
{
    ...
    else if (p->lLink != nullptr && p->rLink == nullptr) // link 1 has a node
    {
...
{
    //If the node is not empty, swap the sub trees then traverse until another not is not empty
    nodeType<elemType> *temp; // Used for swapping
